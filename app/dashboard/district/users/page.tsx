...
const roleOptions = ['pastor','registrar','accountant']
const roleColors: Record<string, string> = {
  pastor: 'bg-[#1a3a2a] text-white',
  registrar: 'bg-blue-600 text-white',
  accountant: 'bg-[#c9a84c] text-[#1a3a2a]',
}
const emptyForm = { name: '', email: '', password: '', userRole: 'pastor', churchId: '' }
...
export default function DistrictUsersPage() {
  ...
  const load = async () => {
    ...
    const [u, c] = await Promise.all([
      ...
      fetch(`/api/manage/churches?district=${user.districtId}`).then(r => r.json()),
    ])
    setUsers(u.users ?? [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/manage/users', {
      ...
      body: JSON.stringify({ ...form, districtId: user.districtId, provinceId: user.provinceId }),
    })
  ...
  const columns = [
    ...
    { key: 'church', label: 'Church', render: (r: any) => r.church?.name ?? '—' },
    ...
    <DashboardLayout navItems={navItems} roleLabel="District Admin" roleColor="bg-blue-600 text-white">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Users in {user?.districtName}</h1>
            <p className="text-foreground/60 mt-1">Manage church staff in this district</p>
          </div>
      ...
      {showModal && (
        <Modal title="Add Church Staff" onClose={() => { setShowModal(false); setError('') }}>
