...
export default function DistrictChurchesPage() {
  ...
  const load = () => {
    ...
    fetch(`/api/manage/churches?district=${user.districtId}`).then(r => r.json()).then(d => setChurches(d.churches ?? []))
  }
  useEffect(() => { load() }, [user?.districtId])
  ...
  const columns = [
    ...
    { key: 'village', label: 'Village', render: (r: any) => r.village ?? '—' },
    ...
    <DashboardLayout navItems={navItems} roleLabel="District Admin" roleColor="bg-blue-600 text-white">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Churches in {user?.districtName}</h1>
            <p className="text-foreground/60 mt-1">{churches.length} churches in this district</p>
          </div>
        ...
        <DataTable columns={columns} data={filtered} emptyMessage="No churches yet in this district" />
      </div>

      {showModal && (
        <Modal title="Add Church in {user?.districtName}" onClose={() => setShowModal(false)}>
          <ChurchForm
            presetProvinceId={user?.provinceId}
            presetDistrictId={user?.districtId}
