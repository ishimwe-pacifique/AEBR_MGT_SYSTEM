...
import { LayoutDashboard, Building2, Users, TrendingUp, MapPin, BarChart3 } from 'lucide-react'
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Legend } from 'recharts'

const navItems = [
  { label: 'Overview', href: '/dashboard/province', icon: LayoutDashboard },
  { label: 'Districts', href: '/dashboard/province', icon: MapPin },
  { label: 'Churches', href: '/dashboard/province', icon: Building2 },
    ...
    <DashboardLayout navItems={navItems} roleLabel="Province Admin" roleColor="bg-purple-600 text-white">
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-6">
          ...
          <StatCard title="Income (RWF)" value={(data?.stats.totalIncome ?? 0).toLocaleString()} icon={TrendingUp} color="green" />
          <StatCard title="Expenses (RWF)" value={(data?.stats.totalExpenses ?? 0).toLocaleString()} icon={TrendingUp} color="red" />
        </div>

        {data?.districts?.length > 0 && (
          <div className="bg-white rounded-2xl p-6 border border-border shadow-sm">
            <h2 className="text-lg font-semibold text-foreground mb-6">Districts Comparison</h2>
        ...
        <div>
          <h2 className="text-lg font-semibold text-foreground mb-4">District Breakdown</h2>
