...
import { LayoutDashboard, Building2, Users, TrendingUp, MapPin, Globe, DollarSign, BarChart3, Settings, UserCog } from 'lucide-react'
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          {data?.provinces?.length > 0 && (
            <div className="bg-white rounded-2xl p-6 border border-border shadow-sm">
              <h2 className="text-lg font-semibold mb-6">Members by Province</h2>
          ...
          {data?.provinces?.length > 0 && (
            <div className="bg-white rounded-2xl p-6 border border-border shadow-sm">
              <h2 className="text-lg font-semibold mb-6">Income Distribution</h2>
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={data.provinces} dataKey="income" nameKey="name" cx="50%" cy="50%" outerRadius={90}
                    label={({ name, percent }) => `${name.split(' ')[0]} ${(percent*100).toFixed(0)}%`}>
                    {data.provinces.map((_: any, i: number) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                  </Pie>
